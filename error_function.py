...
#! 计算误差函数
def compute_loss(fwd_matrix, laser_matrix, weights=None):
    # 如果未提供权重，则使用默认权重
    if weights is None:
        weights = ERROR_WEIGHTS
    
    # 计算位置误差
    position1 = fwd_matrix[0:3, 3]
    position2 = laser_matrix[0:3, 3]
    position_error = (position2 - position1) * weights[0:3]

    # 计算旋转误差
    rotation1 = fwd_matrix[0:3, 0:3]
    ...
    # 转换为轴角表示
    rotvec = Rotation.from_matrix(rotation_diff).as_rotvec()
...
if __name__ == "__main__":
    ...
    for i in range(len(fwd_tool_matrix)):
        error_norm, error_vector = compute_loss(fwd_tool_matrix[i], laser_tool_matrix[i])
        print(error_norm)
        print(error_vector)
